...
#define kStrShortcutQuestion _I10NS(@"addShortcut")
#define kStrShortcutInfo _I10NS(@"addShortcutInfo")
#define kStrShortcutOK _I10NS(@"addShortcutOK")
#define kStrShortcutNotOK _I10NS(@"addShortcutNotOK")
...
static void CreateShortcut(NSString *destinationPath);
...
void PFMoveToApplicationsFolderIfNecessary(void) {
	...
	NSAlert *alert = [[[NSAlert alloc] init] autorelease];
	{
		...
		NSTextView *infoView = [[NSTextView alloc] init];
		NSMutableParagraphStyle * myStyle2 = [[NSMutableParagraphStyle alloc] init];
		[myStyle2 setLineSpacing:5.0];
		[myStyle2 setMinimumLineHeight:14.0];
		...
		[infoView setDrawsBackground:false];
		//[customView addSubview:infoView];
		[alert setAccessoryView:infoView];
...
//- (void)CreateShortcut:(NSString *)destinationPath{
static void CreateShortcut(NSString *destinationPath){
	NSAlert *a = NSAlert.new;
	[a setMessageText: kStrShortcutQuestion];
	 
	/*NSView *customView = [[NSView alloc] init];
	[customView setFrame:NSMakeRect(0, 0, 300, 200)];
	[customView setWantsLayer: YES];
	[customView.layer setBackgroundColor: [NSColor redColor].CGColor];*/
	
	

	/*NSTextView *titleView = [[NSTextView alloc] init];
	NSMutableParagraphStyle * myStyle = [[NSMutableParagraphStyle alloc] init];
	[myStyle setLineSpacing:8.0];
	[myStyle setMinimumLineHeight:20.0];
	[myStyle setMaximumLineHeight:20.0];
	[titleView setDefaultParagraphStyle:myStyle];
	
	[titleView setFrame: NSMakeRect(0,0,300,80)];
	[titleView setString: kStrShortcutQuestion];
	[titleView setFont:[NSFont systemFontOfSize:20]];
	...
	[infoView setDrawsBackground:false];
	//[customView addSubview:infoView];
	[a setAccessoryView:infoView];
	//[a setInformativeText:kStrShortcutInfo];
	[a addButtonWithTitle:kStrShortcutOK];
	[a addButtonWithTitle:kStrShortcutNotOK];
	
	
	if ([a runModal] == NSAlertFirstButtonReturn) {
		NSArray * paths = NSSearchPathForDirectoriesInDomains (NSDesktopDirectory, NSUserDomainMask, YES);
		NSString * desktopPath = [paths objectAtIndex:0];
		NSString * cmd = [NSString stringWithFormat:@"tell application \"Finder\" to set name of (make alias file to (POSIX file \"%@\" as alias) at POSIX file \"%@\") to \"Bridgestars\"", destinationPath, desktopPath];
		NSAppleScript *appleScript = [[[NSAppleScript alloc] initWithSource:
									   [NSString stringWithFormat:@"%@", cmd]] autorelease];
		NSDictionary *errorDict = nil;
		NSAppleEventDescriptor *scriptResult = [appleScript executeAndReturnError:&errorDict];
		if (scriptResult == nil) {
			NSLog(@"Shortcut AppleScript error: %@", errorDict);
		}
	}
}
